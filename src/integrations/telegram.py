...
class TelegramNotifier:
    ...
    def send_photo(self, image_data: bytes, caption: str = "", parse_mode: str = "Markdown") -> Dict:
        """
        Send a photo with optional caption via Telegram
        ...
        """
        if not self.is_configured:
            logger.warning("Telegram not configured - cannot send photo")
        ...
        files = {
            'photo': ('chart.png', image_data, 'image/png')
        }
        ...
        try:
            ...
            # Try sending without parse_mode if we get a formatting error
            if "can't parse entities" in str(e).lower():
                logger.warning("Retrying photo without Markdown parsing")
                try:
                    ...
                    logger.error(f"Failed to send plain photo: {e2}")
            return {"error": str(e)}
    ...
    def set_bot_instance(self, bot_instance) -> None:
        ...
        if self._command_handler:
            ...
            self._command_handler.set_telegram_notifier(self)
            logger.info("Bot instance and telegram notifier set for command handler")
